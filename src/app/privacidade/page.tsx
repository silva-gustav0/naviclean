      ...
      <main className="pt-20">
        <div className="max-w-6xl mx-auto px-6 py-12 flex gap-10">
          <aside className="hidden lg:block w-56 shrink-0">
            <div className="sticky top-24">
              ...
              <nav className="space-y-1">
                {sections.map((s) => (
                  <a key={s.id} href={`#${s.id}`} className="block text-sm text-slate-600 hover:text-[#0D3A6B] py-1 hover:underline transition-colors">
                    {s.title}
                  </a>
                ))}
              </nav>
          ...
          <div className="flex-1 max-w-3xl">
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-[#0F172A] mb-2">Política de Privacidade</h1>
              <p className="text-sm text-slate-500">Última atualização: 1 de janeiro de 2025 — em conformidade com a LGPD</p>
            </div>
            ...
            <div className="mt-12 pt-8 border-t">
              <p className="text-xs text-slate-400">NaviClin Tecnologia Ltda — dpo@naviclin.com</p>
            </div>
          </div>
        </div>
      </main>
